...
class Extraction:
    ...
    @staticmethod
    def pdf_to_json(sheetfilename, jsondirectorypath, teacherstatus):

        # Load the Excel file
        file_path = '../' + sheetfilename # Replace with your actual file path
        ...
        for collection_name, data in collections.items():
            with open(f'{jsondirectorypath}/{collection_name}.json', 'w', encoding='utf-8') as json_file:
                json.dump(data, json_file, indent=4, ensure_ascii=False)

        print(f"JSON files generated successfully in the {jsondirectorypath} directory.")
